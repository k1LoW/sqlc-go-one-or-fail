...
func (q *Queries) CreateAuthor(ctx context.Context, db DBTX, arg CreateAuthorParams) (Author, error) {
	row := db.QueryRowContext(ctx, createAuthor, arg.Name, arg.Bio)
	var i Author
...
func (q *Queries) GetAuthor(ctx context.Context, db DBTX, id int64) (Author, error) {
	row := db.QueryRowContext(ctx, getAuthor, id)
	var i Author
	err := row.Scan(&i.ID, &i.Name, &i.Bio)
	return i, err
}
...
func (q *Queries) GetAuthorBio(ctx context.Context, db DBTX, id int64) (sql.NullString, error) {
	row := db.QueryRowContext(ctx, getAuthorBio, id)
...
func (q *Queries) GetAuthorByName(ctx context.Context, db DBTX, name string) (Author, error) {
	row := db.QueryRowContext(ctx, getAuthorByName, name)
	var i Author
	err := row.Scan(&i.ID, &i.Name, &i.Bio)
	return i, err
}
...
func (q *Queries) GetAuthorName(ctx context.Context, db DBTX, id int64) (string, error) {
	row := db.QueryRowContext(ctx, getAuthorName, id)
	var name string
	err := row.Scan(&name)
	return name, err
}
...
func (q *Queries) GetAuthorNameAndBio(ctx context.Context, db DBTX, id int64) (GetAuthorNameAndBioRow, error) {
	row := db.QueryRowContext(ctx, getAuthorNameAndBio, id)
